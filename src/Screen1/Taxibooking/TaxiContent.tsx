  ...
  useEffect(() => {
    const handleDriverLiveLocationUpdate = (data: { driverId: string; lat: number; lng: number; status?: string }) => {
      ...
      if (currentRideId && acceptedDriver && data.driverId === acceptedDriver.driverId) {
        ...
        // CRITICAL: Update driverLocation state for the map marker
        const driverCoords = { latitude: data.lat, longitude: data.lng };
        setDriverLocation(driverCoords);
        
        // Also update the nearbyDrivers array
  ...
  useEffect(() => {
    ...
    const rideAccepted = (data: any) => {
      if (data.rideId === currentRideId) {
        ...
        // CRITICAL: Set initial driver location
        if (found) {
          setDriverLocation({
        ...
        console.log(`✅ Ride ${currentRideId} accepted by driver:`, data);
    ...
    // CRITICAL: Add this listener for driver location updates
    const driverLocationUpdate = (data: any) => {
      console.log('📍 Received driver location update:', data);
    ...
    socket.on("driverLocationUpdate", driverLocationUpdate); // CRITICAL: Ensure this event is listened to
    ...
    return () => {
      ...
      socket.off("driverLocationUpdate", driverLocationUpdate); // CRITICAL: Remove listener on cleanup
...
useEffect(() => {
  ...
  // Specific listener for driver location updates
              ...
              {driverLocation && (
                ...
                >
                  <View style={styles.driverMarkerContainer}>
                    <View style={[styles.redDotMarker, { backgroundColor: rideStatus === "onTheWay" || rideStatus === "arrived" || rideStatus === "started" ? '#FF6B00' : '#19d522ff' }]} />
...
//       // Calculate distance if needed
//       if (lastCoord) {
//         const dist = haversine(lastCoord, { latitude: data.lat, longitude: data.lng });
//         setTravelledKm(prev => prev + dist / 1000);
//       }
//       setLastCoord({ latitude: data.lat, longitude: data.lng });
//     }
//     return; // Ignore other drivers during active ride
//   }
